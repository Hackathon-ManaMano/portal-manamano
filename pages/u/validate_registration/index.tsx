...
import { ApproveRepproveEnum } from "../../../enums/aprova_reprova_cadastro_enum";
...
export default function ValidateRegistration() {
    ...
    const showToast = (action: ApproveRepproveEnum) => {
        let message = "Usuário aprovado com sucesso";
        let severity = "Aprovado";
        if (action == ApproveRepproveEnum.REPPROVE) {
    ...
    const confirm = (
        ...
        action: ApproveRepproveEnum,
        rowData: Empreendedora
    ) => {
        confirmPopup({
            target: event.currentTarget,
            message: `Tem certeza que quer ${action.toLowerCase()} o cadastro de ${
                rowData.nome
            }?`,
            ...
            acceptClassName:
                action == ApproveRepproveEnum.APPROVE
                    ? "p-button-primary"
                    : "p-button-danger",
            ...
            <div className="flex justify-content-around gap-2 align-items-center">
                <Button
                    ...
                    onClick={(event: MouseEvent<HTMLButtonElement>) =>
                        confirm(event, ApproveRepproveEnum.APPROVE, rowData)
                    }
                />
                <Button
                    ...
                    tooltipOptions={{ position: "left" }}
                    onClick={(event: MouseEvent<HTMLButtonElement>) =>
                        confirm(event, ApproveRepproveEnum.REPPROVE, rowData)
                    }
                />
            </div>
        );
